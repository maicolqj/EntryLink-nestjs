...
export class VisitorsService {
  ...
  async blacklist(
    ...
    this.logger.warn(
      `Visitante bloqueado: ${visitor.id} — ${visitor.fullName} por ${currentUser.sub}`,
    );
    ...
    this.notifyBlacklisted(saved, currentUser).catch((err) =>
      this.logger.warn(
        `Error al notificar visitante en lista negra ${saved.id}: ${err?.message}`,
      ),
    );
  ...
  async removeFromBlacklist(
    ...
    this.logger.log(
      `Visitante desbloqueado: ${visitor.id} por ${currentUser.sub}`,
    );
    return this.visitorRepo.save(visitor);
  }
