...
const questionOf = (partial: Partial<VotingQuestion> = {}): VotingQuestion =>
  ({
    ...
    meeting: {
      id: 'm-1',
      kind: VotingMeetingKind.ASAMBLEA,
...
const build = (question: VotingQuestion | null, opts: BuildOpts = {}) => {
  ...
  const service = new VotingService(
    ...
    { findById: jest.fn().mockResolvedValue({ id: 'complex-1' }) } as never,
    {
      ...
      findActiveUserIdsByComplexInternal: jest
        .fn()
        .mockResolvedValue(['res-user']),
      findActiveResidentByUserIdInternal: jest.fn().mockResolvedValue(
        opts.resident === undefined
          ? {
              id: 'resident-1',
              unitId: 'unit-301',
              unit: { coefficient: 0.0125 },
            }
          : opts.resident,
      ),
    } as never,
...
describe('VotingService — votar', () => {
  ...
  it('una unidad, un voto: con peso 1 aunque tenga coeficiente', async () => {
    const { service, inserted } = build(
      questionOf({ weighting: VoteWeighting.UNIT }),
    );
  ...
  it('el segundo residente de la misma unidad no puede votar', async () => {
    ...
    await expect(
      service.castVote('q-1', 'opt-si', resident('otro-de-la-unidad')),
    ).rejects.toThrow('Tu unidad ya votó en esta pregunta');
  });

  it('no se vota en una pregunta cerrada', async () => {
    const { service } = build(
      questionOf({ status: VotingQuestionStatus.CLOSED }),
    );

    await expect(service.castVote('q-1', 'opt-si', resident())).rejects.toThrow(
  ...
  it('una opción de otra pregunta no vale', async () => {
    ...
    await expect(
      service.castVote('q-1', 'opt-ajena', resident()),
  ...
  it('el consejero con voz pero sin voto no vota', async () => {
    ...
    await expect(
      service.castVote('q-1', 'opt-si', resident('c-2')),
    ).rejects.toThrow('En el consejo tienes voz pero no voto');
    await expect(service.isVoiceOnly(consejo, resident('c-2'))).resolves.toBe(
      true,
    );
    await expect(
      service.viewerBallot(consejo, resident('c-2')),
    ).resolves.toEqual({ eligible: false, optionId: null });
  });
  ...
  it('en el consejo solo votan consejeros, uno por persona', async () => {
    ...
    await expect(
      noCouncil.castVote('q-1', 'opt-si', resident()),
    ).rejects.toThrow('Esta votación es del consejo de administración');

    const { service, inserted } = build(consejo, {
      isCouncil: true,
      councilIds: ['consejero-1'],
      councilEnabled: true,
    });
    await service.castVote('q-1', 'opt-si', resident('consejero-1'));
    expect(inserted[0]).toMatchObject({
      voterKey: 'user:consejero-1',
      weight: 1,
      unitId: null,
    });
  });
...
describe('VotingService — abrir', () => {
  it('por coeficiente no abre si a alguna unidad le falta el coeficiente', async () => {
    const { service } = build(
      questionOf({ status: VotingQuestionStatus.DRAFT }),
      {
        units: [
          { id: 'u1', coefficient: 0.5 },
          { id: 'u2', coefficient: null },
        ],
      },
    );

    await expect(service.openQuestion('q-1', admin())).rejects.toThrow(
      '1 unidad(es) no tienen coeficiente',
    );
  });
  ...
  it('no abre si los residentes no pueden ver el módulo', async () => {
    ...
    await expect(service.openQuestion('q-1', admin())).rejects.toThrow(
      'Activa las asambleas para los residentes',
    );
  });
  ...
  it('una pregunta del consejo exige el interruptor del consejo, no el de residentes', async () => {
    ...
    const { service: off } = build(questionOf(consejo), {
      enabled: true,
      councilEnabled: false,
      councilIds: ['c-1'],
    });
    await expect(off.openQuestion('q-1', admin())).rejects.toThrow(
      'Activa las reuniones del consejo',
    );

    // Solo el consejo encendido basta: los residentes no tienen que ver nada.
    const { service: on } = build(questionOf(consejo), {
      enabled: false,
      councilEnabled: true,
      councilIds: ['c-1'],
    });
  ...
  it('abrir le avisa a la app de los residentes en tiempo real', async () => {
    ...
    expect(socket.emitToUsers).toHaveBeenCalledWith(
      ['res-user'],
      'voting:updated',
      expect.objectContaining({ status: VotingQuestionStatus.OPEN }),
    );
  });
  ...
  describe('interruptores separados', () => {
    it('con solo el consejo encendido, un residente que no es del consejo no ve el módulo', async () => {
      ...
      await expect(service.isEnabled('complex-1', resident())).resolves.toBe(
        false,
      );
      await expect(
        service.findMyMeetings('complex-1', resident()),
    ...
    it('con solo el consejo encendido, el consejero ve únicamente las reuniones del consejo', async () => {
      const { service, meetingRepo } = build(null, {
        enabled: false,
        councilEnabled: true,
        isCouncil: true,
      });
      ...
      await expect(
        service.isEnabled('complex-1', resident('c-1')),
      ).resolves.toBe(true);
      await service.findMyMeetings('complex-1', resident('c-1'));

      expect(meetingRepo.find.mock.calls[0][0].where.kind.value).toEqual([
        VotingMeetingKind.CONSEJO,
      ]);
    });
    ...
    it('con las asambleas apagadas, la pregunta de una asamblea no se alcanza', async () => {
      ...
      await expect(
        service.castVote('q-1', 'opt-si', resident('c-1')),
      ).rejects.toThrow('Las votaciones no están habilitadas en tu conjunto');
    });
  });
  ...
  it('al abrir avisa a los residentes', async () => {
    ...
    expect(notify).toHaveBeenCalledWith(
      expect.objectContaining({ userIds: ['res-user'], entityType: 'voting' }),
    );
  });
});
...
describe('VotingService — resultados', () => {
  ...
  it('el porcentaje es por peso, no por cantidad de votos', async () => {
    ...
    const results = await service.results(
      questionOf({
        status: VotingQuestionStatus.CLOSED,
        eligibleCount: 10,
        eligibleWeight: 1,
      }),
      resident(),
  ...
  it('en voto secreto la administración ve quién participó, no qué votó', async () => {
    const { service } = build(null, {
      ballots,
      units: [{ id: 'u1', coefficient: 1 }],
    });

    const results = await service.results(
      questionOf({ secrecy: VoteSecrecy.SECRET }),
      admin(),
    );
