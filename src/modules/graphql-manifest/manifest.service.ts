  ...
  async onModuleInit(): Promise<void> {
    ...
    if (existsSync(filePath)) {
      try {
        ...
        this.store = entries;
        // Persist into Redis so subsequent instances (horizontal scale / restarts)
      ...
      this.logger.warn(`Redis empty and ${MANIFEST_FILE} not found. All queries will be rejected in production until POST /graphql-manifest/sync is called.`);
    }
  }
