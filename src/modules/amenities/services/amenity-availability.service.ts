...
export class AmenityAvailabilityService {
  ...
  async getAvailability(
    ...
    const totalDays = Math.round((toDate.getTime() - fromDate.getTime()) / (24 * 60 * MINUTE_MS)) + 1;
    ...
    const [schedules, blackouts, bookings, exceptions] = await Promise.all([
      ...
      this.loadExceptions(amenity.id, this.formatLocalDate(rangeStart), this.formatLocalDate(rangeEnd)),
    ]);

    const now         = new Date();
    const earliest    = new Date(now.getTime() + amenity.minAdvanceDays * 24 * 60 * MINUTE_MS);
    const latestStart = this.endOfDay(this.addDays(this.startOfDay(now), amenity.advanceBookingDays));

    const days: AmenityAvailabilityDay[] = [];

    for (let d = 0; d < totalDays; d++) {
      const dayStart = this.addDays(fromDate, d);
      days.push(this.buildDay(amenity, dayStart, schedules, blackouts, bookings, exceptions, earliest, latestStart));
    }
   ...
   */
  async getOpenWindowsForDay(amenityId: string, dayStart: Date): Promise<AmenityTimeWindow[]> {
    ...
    // muestra cerrado.
    return this.openWindowsForDay(dayStart, schedules, blackouts, exceptions[0] ?? null);
  }
  ...
  async getContinuousWindowsForDay(
    ...
    const [schedules, blackouts, exceptions] = await Promise.all([
      ...
      this.loadExceptions(amenityId, this.formatLocalDate(dayStart), this.formatLocalDate(rangeEnd)),
    ]);
    ...
    for (let d = 0; d <= lookaheadDays; d++) {
      ...
      windows.push(
        ...this.openWindowsForDay(day, schedules, blackouts, exceptions.find(e => e.date === date) ?? null),
      );
    }
  ...
  private loadBlackouts(amenityId: string, from: Date, to: Date): Promise<AmenityBlackout[]> {
  ...
  private loadExceptions(amenityId: string, from: string, to: string): Promise<AmenityScheduleException[]> {
  ...
  private loadActiveBookings(amenityId: string, from: Date, to: Date): Promise<AmenityBooking[]> {
  ...
  private buildDay(
    ...
    if (amenity.status !== AmenityStatus.ACTIVE) return closed(ClosedReason.ZONA_INACTIVA);
    ...
    if (dayEnd <= earliest || dayStart > latestStart) return closed(ClosedReason.FUERA_DE_VENTANA);
    ...
    const openWindows = this.openWindowsForDay(dayStart, schedules, blackouts, exception);
    ...
    const slots = amenity.durationUnit === AmenityDurationUnit.DAYS
      ? this.buildWholeDaySlot(amenity, dayStart, dayEnd, dayBookings, earliest, latestStart)
      : amenity.bookingMode === AmenityBookingMode.SLOT
        ...
        ? this.buildSlots(amenity, openWindows, bookings, earliest, latestStart)
        : [];
  ...
  private openWindowsForDay(
    ...
    const raw = source
      .map(s => {
        ...
        const endAt = sameDayEnd > startAt ? sameDayEnd : this.combine(this.addDays(dayStart, 1), s.closeTime);
  ...
  private buildSlots(
    ...
    for (const window of openWindows) {
      for (let start = window.startAt.getTime(); start + stepMs <= window.endAt.getTime(); start += stepMs) {
        const startAt = new Date(start);
        const endAt   = new Date(start + stepMs);

        const capacityUsed = dayBookings.filter(b => b.startAt < endAt && b.endAt > startAt).length;
        ...
        slots.push({
          ...
          capacityUsed,
          isAvailable: withinWindow && capacityUsed < amenity.maxSimultaneousBookings,
        });
      }
    }
  ...
  private buildWholeDaySlot(
    ...
    const capacityUsed = dayBookings.filter(b => b.startAt < dayEnd && b.endAt > dayStart).length;
  ...
  private buildBusyRanges(dayBookings: AmenityBooking[]): AmenityBusyRange[] {
    ...
    for (const b of dayBookings) {
      ...
      if (existing) {
        ...
        byRange.set(key, { startAt: b.startAt, endAt: b.endAt, bookingsCount: 1 });
      }
    }

    return [...byRange.values()].sort((a, b) => a.startAt.getTime() - b.startAt.getTime());
  }
  ...
  private mergeWindows(windows: AmenityTimeWindow[]): AmenityTimeWindow[] {
    ...
    const sorted = [...windows].sort((a, b) => a.startAt.getTime() - b.startAt.getTime());
