...
export class UnitService {
  ...
  async create(
    ...
    const response = await this.complexService.findById(
      input.complexId,
      currentUser,
    );
    ...
    const whereClause = input.buildingId
      ? {
          ...
          number: input.number.toUpperCase().trim(),
          deletedAt: IsNull(),
        }
      : {
          complexId: input.complexId,
    ...
    const existing = await this.unitRepo.findOne({ where: whereClause });
    if (existing) {
      throw new CustomError({
    ...
    this.logger.log(
      `Unidad creada: ${saved.id} — N°${saved.number} en complejo ${input.complexId}`,
    );
    return saved;
  }
  ...
  async findByComplex(
    ...
    const cached = await this.cacheService.get<PaginatedUnitsResponse>({
      key: cacheKey,
    });
    ...
    if (buildingId)
      qb.andWhere('unit.buildingId = :buildingId', { buildingId });
    if (status) qb.andWhere('unit.status = :status', { status });
    if (term) {
      qb.andWhere('(unit.number ILIKE :term OR building.name ILIKE :term)', {
        term: `%${term}%`,
      });
    }
  ...
  async findById(id: string, currentUser: JwtAccessPayload): Promise<Unit> {
    const unit = await this.unitRepo.findOne({
  ...
  async update(
    ...
    if (
      input.status === UnitStatus.AVAILABLE &&
      unit.status === UnitStatus.OCCUPIED
    ) {
      throw new CustomError({
        message:
          'No puedes liberar una unidad ocupada directamente. Gestiona el traslado desde el módulo de residentes.',
        statusCode: HttpStatus.BAD_REQUEST,
  ...
  async remove(
    ...
    if (unit.status === UnitStatus.OCCUPIED) {
      throw new CustomError({
        message:
          'No se puede eliminar una unidad que está ocupada. Gestiona el traslado primero.',
        statusCode: HttpStatus.BAD_REQUEST,
        errorCode: ComplexErrorCode.UNIT_IS_OCCUPIED,
      });
    }
    ...
    return {
      success: true,
      message: `Unidad N°${unit.number} eliminada correctamente`,
    };
  }
  ...
  async setOccupied(id: string, occupied: boolean): Promise<void> {
    ...
    if (complexId)
      await this.cacheService.deleteByPrefix(BK.unit.prefix(complexId));
  }
}
