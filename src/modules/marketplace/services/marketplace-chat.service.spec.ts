...
describe('MarketplaceChatService — avisos ya vistos', () => {
  it('abrir la conversación da por leídos sus avisos de mensaje', async () => {
    const { service, notificationsService } = buildHarness();

    await service.markRead('conv-1', OWNER);
    await flush();

    expect(notificationsService.markEntityAsReadFor).toHaveBeenCalledWith(
      'owner-1',
  ...
  it('escribir también: quien responde ya leyó lo anterior', async () => {
    const { service, notificationsService } = buildHarness();

    await service.send('conv-1', 'Sí, sigue disponible', OWNER);
    await flush();
