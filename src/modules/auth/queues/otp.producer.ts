...
export class OtpProducer {
  ...
  constructor(@InjectQueue(OTP_QUEUE_NAME) private readonly otpQueue: Queue) {}
  ...
  async sendOtp(payload: SendOtpJobPayload): Promise<void> {
    ...
    this.logger.log(
      `OTP job enqueued for phone: ${payload.phoneNumber.replace(/\d{6}$/, '******')}`,
    );
  }
  ...
  async sendSystemCode(payload: SendSystemCodeJobPayload): Promise<void> {
    ...
    this.logger.log(
      `System-code job enqueued for phone: ${payload.phoneNumber.replace(/\d{6}$/, '******')}`,
    );
  }
}
