...
@Catch()
export class UniversalExceptionFilter
  implements ExceptionFilter, GqlExceptionFilter
{
  ...
  private describeClient(request: any): string {
    ...
    const userAgent = this.sanitizeForLog(
  ...
  private describeBodyParserError(
    exception: unknown,
  ): { status: number; message: string; errorCode: GeneralErrorCode } | null {
    ...
    switch (err.type) {
      case 'entity.too.large':
        return {
          ...
          message:
            'El contenido enviado supera el tamaño máximo permitido. Sube un archivo más liviano.',
  ...
  private handleHttpException(exception: unknown, host: ArgumentsHost) {
    ...
    try {
      ...
      this.logHttpFailure(
        status,
        request,
        JSON.stringify({ message, errorCode }),
      );

      response.status(status).json(errorResponse);
    } catch (error) {
      this.logger.error(
        'Error in HTTP exception filter',
        error instanceof Error ? error.stack : String(error),
      );
      response.status(HttpStatus.INTERNAL_SERVER_ERROR).json({
        statusCode: HttpStatus.INTERNAL_SERVER_ERROR,
        message:
          'Internal server error occurred while processing the original error',
        timestamp: new Date().toISOString(),
        path: request.url,
        method: request.method,
      });
  ...
  private handleGraphQLException(exception: unknown, host: ArgumentsHost) {
    ...
    try {
      ...
      if (bodyParserError) {
        ...
        this.logger.error(
          `Unhandled GraphQL error: ${exception.message}`,
          exception.stack,
        );
        message = 'Internal server error';
      }
      ...
      this.logger.error(
        'Error in GraphQL exception filter',
        error instanceof Error ? error.stack : String(error),
      );
