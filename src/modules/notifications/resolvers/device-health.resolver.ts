...
export class DeviceHealthResolver {
  ...
  async runPushHealthCheck(
    ...
    const { sentAt } = await this.deviceHealthService.sendHealthCheck(
      subscription.id,
    );
  ...
  async devicePushHealth(
    ...
    return {
      ...
      lastCheckAcknowledged:
        !!health.lastTestAckAt &&
        !!health.lastTestSentAt &&
  ...
  async reportDevicePermissions(
    @Args('deviceToken') deviceToken: string,
    @CurrentUser() currentUser: JwtAccessPayload,
    @Args('hasNotificationPermission', { nullable: true })
    hasNotificationPermission?: boolean,
    @Args('hasBatteryOptimizationDisabled', { nullable: true })
    hasBatteryOptimizationDisabled?: boolean,
    @Args('hasFullScreenIntentPermission', { nullable: true })
    hasFullScreenIntentPermission?: boolean,
    @Args('onboardingCompleted', { nullable: true })
    ...
    const health = await this.deviceHealthService.reportPermissions(
      subscription.id,
      {
