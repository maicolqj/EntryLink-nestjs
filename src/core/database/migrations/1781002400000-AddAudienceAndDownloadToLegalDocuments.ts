...
export class AddAudienceAndDownloadToLegalDocuments1781002400000 implements MigrationInterface {
  public async up(queryRunner: QueryRunner): Promise<void> {
    ...
    `);
    await queryRunner.query(
      `ALTER TABLE "legal_documents" ADD COLUMN IF NOT EXISTS "download_file_url" text`,
    );
    await queryRunner.query(
      `ALTER TABLE "legal_documents" ADD COLUMN IF NOT EXISTS "download_file_name" varchar(255)`,
    );
    await queryRunner.query(
      `ALTER TABLE "legal_documents" ADD COLUMN IF NOT EXISTS "download_file_public_id" text`,
    );
  ...
  public async down(queryRunner: QueryRunner): Promise<void> {
    await queryRunner.query(
      `ALTER TABLE "legal_documents" DROP COLUMN IF EXISTS "download_file_public_id"`,
    );
    await queryRunner.query(
      `ALTER TABLE "legal_documents" DROP COLUMN IF EXISTS "download_file_name"`,
    );
    await queryRunner.query(
      `ALTER TABLE "legal_documents" DROP COLUMN IF EXISTS "download_file_url"`,
    );
    await queryRunner.query(
      `ALTER TABLE "legal_documents" DROP COLUMN IF EXISTS "is_downloadable"`,
    );
    await queryRunner.query(
      `ALTER TABLE "legal_documents" DROP COLUMN IF EXISTS "audience"`,
    );
    await queryRunner.query(
      `DROP TYPE IF EXISTS "legal_documents_audience_enum"`,
    );
  }
}
