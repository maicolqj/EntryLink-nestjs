      ...
      useFactory: async (configService: ConfigService) => {
        ...
        try {
            ...
            JSON.stringify(
              {
                ...
                maxRetriesPerRequest:
                  bullmqConfig.connection.maxRetriesPerRequest,
          ...
          const missingFields = requiredFields.filter(
            (field) => !bullmqConfig.connection[field],
          );

          if (missingFields.length > 0) {
            throw new Error(
              `Missing required BullMQ connection fields: ${missingFields.join(', ')}`,
            );
          }
          ...
          const finalConfig = {
            ...
            connection: {
              ...
              retryDelayOnFailover:
                bullmqConfig.connection.retryDelayOnFailover || 500,
