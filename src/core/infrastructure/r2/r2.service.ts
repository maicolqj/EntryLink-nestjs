...
export class R2StorageService implements OnModuleInit {
  ...
  onModuleInit() {
    ...
    this.publicUrl =
      rawPublicUrl && !rawPublicUrl.startsWith('http')
        ? `https://${rawPublicUrl}`
        : rawPublicUrl;
    ...
    this.logger.debug(
      `R2 bucket ${this.bucketName} — url pública ${this.publicUrl}`,
    );
    ...
    this.logger.log(
      `Cloudflare R2 configurado correctamente — carpeta raíz "${this.appRoot}"`,
    );
  }
  ...
  async uploadBuffer(
    ...
    const contentType =
      EXT_TO_CONTENT_TYPE[format] ?? 'application/octet-stream';
